...
function init() {
    ...
    while (minesNum) {
        var mineIndex = Math.floor(Math.random() * 200);
...
function leftClick(dom) {
    ...
    if (!dom.classList.contains('check')) {
        ...
        if (dom && dom.classList.contains('isLei')) {
            ...
            dom && (dom.innerHTML = n);
            if (n == 0) {
                // if (!dom.classList.contains('zero-check')){
                    for (var i = posX - 1; i <= posX + 1; i++) {
                        for (var j = posY - 1; j <= posY + 1; j++) {
                            if (i == posX && j == posY) {
                                // nearBox.classList.add('zero-check')
                                continue;
                            }
                            var nearBox = document.getElementById(i + '-' + j);

                            if (nearBox) {
                                // if (!nearBox.classList.contains('zero-check')) {
                                    // nearBox.classList.add('zero-check');
                                    leftClick(nearBox);
                                // }
                            }
